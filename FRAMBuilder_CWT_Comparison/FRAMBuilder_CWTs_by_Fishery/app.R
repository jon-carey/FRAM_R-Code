...
ui <- fluidPage(
  ...
  # App title ----
  titlePanel("FRAMBuilder CWTs by Fishery"),
  ...
  sidebarLayout(
    ...
    sidebarPanel(width = 8,
      ...
      selectInput(inputId = "fishery",
                  label = "Select fishery:",
                  choices = as.character(unique(Fish_LUT$FisheryTitle)))
...
server <- function(input, output) {
  ...
  CWT_stock <- reactive({
    CWT_data_long %>%
      filter(FisheryName == input$fishery)
  })
  ...
  output$Plot <- renderPlotly({
    ...
    print(
      ggplotly(
        ggplot(data = CWT_stock(), aes(StockTitle, Recoveries, fill = Version)) +
