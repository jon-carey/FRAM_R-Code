...
* split "overall summary" box plots into multiple plots (PS, ColR, Coastal/Canada?) - things are too crammed currently. replace full stock names with abbreviations?
* pull ERs from TAMMs for PS stocks?
* currently FRAM AEQs are TS3 - is there a way to get annual values so they're more comparable to ERA AEQs? On second thought, I don't think providing AEQ comparisons is of much use.  They're simply a function of mat rates (which we're already comparing) and natural mortality rates (which are essentially the same between FRAM and ERA).  I've commented out AEQ comparisons.
* fill in text
* Include scatter plots of FRAM vs ERA ERs?
...
knitr::include_graphics("C:/Users/jonathan.carey/Documents/FRAM/Calibration_Validation/Chinook/2020/Final Package 7.1/FRAM_ERA_Compare/StkCrosswalk.jpg")
```
...
#### ERA
Maturation rates for ERA stocks are calculated during the cohort analysis on an annual basis for each brood year (by), and age (a), as:

$$MatRate_{by,a} = \frac{TotMatCat_{by,a} + Escape_{by,a}}{Cohort_{by,a} * SurvRte_{a} - TotOcnCat_{by,a}}$$

where TotMatCat is the sum of catch and indicental mortality in all terminal fisheries (encounterin mature fish only) and TotOcnCat is the sum of catch and incidental mortality in all pre-terminal (mixed-maturity) fisheries.

#### FRAM
Unlike in the ERA, where maturation rates are calculated independently for each separate brood year, FRAM maturation rates are considered to be static and are an output of the base period calibration.  Further, they are calculated separately for each Chinook FRAM time step (e.g., Oct-Apr, May-Jun, Jul-Sep).  In order to provide maturation rates that are comparable to then annual rates for each ERA stock, annual maturation rates were calculated from the time step-specific rates for each FRAM stock (s) and age (a) as:
