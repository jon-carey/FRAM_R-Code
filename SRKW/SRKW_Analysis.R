...
# Identify proportion of stock abundance in coastal and inland regions
ppnCoastal <- StkDist[ ,c(1,4)]
ppnInland <- StkDist[ ,c(1,5)]
...
colnames(kCal_to_Need) <- c("Year", "Run", "DietComp", "TimeStep", "Min_DPER", 
                            "Max_DPER")
...
for(i in minYr:maxYr) {
    ...
    for(j in 2:3) {
        ...
        cohort$InlandAbundance <- cohort$MidCohort.sum * cohort$ppnInside
